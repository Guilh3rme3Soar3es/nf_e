...
{
	public class SenderRegisterCommand
	{
		...
		class Validator : AbstractValidator<SenderRegisterCommand>
		{
			public Validator()
			{
				...
				RuleFor(r => r.StateRegistration)
					.NotEmpty().WithMessage("Emitente com inscrição estadual não informado.")
					.MaximumLength(15).WithMessage("Emitente com inscrição estadual maior que 15 caracteres.");

				RuleFor(r => r.MunicipalRegistration)
					.NotEmpty().WithMessage("Emitente com inscrição municipal não informado.")
